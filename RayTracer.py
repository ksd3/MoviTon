...
def image(a,theta0,frame_counter,radial_photons=50,angular_photons=200,r_out=20,rest_wavelength=550,wavelength_function=None,unobservable_to_gray=False,set_intensity=False,print_progress=False):
    ...
    files_exist=os.path.isfile("image_x_coord"+save_string+".npy")
    files_exist=files_exist and os.path.isfile("image_y_coord"+save_string+".npy")
    files_exist=files_exist and os.path.isfile("y_physics"+save_string+".npy")
    files_exist=files_exist and os.path.isfile("y_physics"+save_string+".npy")
    files_exist=files_exist and os.path.isfile("redshift_calc"+save_string+".npy")
    ...
    if files_exist:
        ...
        image_x_coord=np.load("image_x_coord"+save_string+".npy")
        image_y_coord=np.load("image_y_coord"+save_string+".npy")
        x_physics=np.load("x_physics"+save_string+".npy")
        y_physics=np.load("y_physics"+save_string+".npy")
        redshift_calcs=np.load("redshift_calc"+save_string+".npy")
        ...
        for i in range(radial_photons):
            for j in range(angular_photons):
                ...
                image_x_coord.append(alpha)
                image_y_coord.append(beta)
                ...
                if rf<rH:
                    ...
                    f=g(rf,a,j,ri,r_out)
                redshift_calcs.append(f)
                x_physics.append(x[-1])
                y_physics.append(y[-1])
                if print_progress:
        ...
        redshift_calcs=[redshift_calcs[i] if not (redshift_calcs[i]<f_min or redshift_calcs[i]> f_max) else np.nan for i in range(radial_photons*angular_photons)]
    
        save_string=["radial_photons=",str(radial_photons),"angular_photons=",str(angular_photons),"a=",str(a),"theta=",str(theta0),"r_out=",str(r_out)]
        save_string="".join(save_string)
        np.save("image_x_coord"+save_string+".npy",image_x_coord)
        np.save("image_y_coord"+save_string+".npy",image_y_coord)
        np.save("x_physics"+save_string+".npy",x_physics)
        np.save("y_physics"+save_string+".npy",y_physics)
        np.save("redshift_calc"+save_string+".npy",redshift_calcs) 
    
    print("Done. ")
    
    #plotting starts here
    if wavelength_function is not None:
        wavelengths=[wavelength_function(x_physics[i],y_physics[i])/redshift_calcs[i] if not np.isnan(redshift_calcs[i]) else np.nan for i in range(radial_photons*angular_photons)]
    else:
        wavelengths=[rest_wavelength/redshift_calcs[i] if not np.isnan(redshift_calcs[i]) else np.nan for i in range(radial_photons*angular_photons)]
    
    imx_lim_u=max([image_x_coord[i] if not np.isnan(wavelengths[i]) else 0 for i in range(radial_photons*angular_photons)])
    imx_lim_l=min([image_x_coord[i] if not np.isnan(wavelengths[i]) else 0 for i in range(radial_photons*angular_photons)])
    imy_lim_u=max([image_y_coord[i] if not np.isnan(wavelengths[i]) else 0 for i in range(radial_photons*angular_photons)])
    imy_lim_l=min([image_y_coord[i] if not np.isnan(wavelengths[i]) else 0 for i in range(radial_photons*angular_photons)])
    ...
    if set_intensity:
        ...
        colors[:,3]=[redshift_calcs[i]**3 if not np.isnan(redshift_calcs[i]) else 0.1 for i in range(radial_photons*angular_photons)]
        colors[:,3]=[.999*(colors[i,3]-min(colors[:,3])) /(max(colors[:,3])-min(colors[:,3]))  for i in range(radial_photons*angular_photons)]
        sc=ax.scatter(image_x_coord,image_y_coord,s=100,color=colors,edgecolors=None)
    else:
        sc=ax.scatter(image_x_coord,image_y_coord,s=100,c=wavelengths,cmap=spectralmap,vmin=349,vmax=781,edgecolors=None)
